...
void qmp_marshal_device_sync_config(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    q_obj_device_sync_config_arg arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_q_obj_device_sync_config_arg_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_DEVICE_SYNC_CONFIG)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_device_sync_config(req_json->str);
    }

    qmp_device_sync_config(arg.id, &err);
    if (err) {
        trace_qmp_exit_device_sync_config(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    trace_qmp_exit_device_sync_config("{}", true);

out:
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_q_obj_device_sync_config_arg_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
