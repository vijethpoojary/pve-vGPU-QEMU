...
void qmp_marshal_query_dump(QDict *args, QObject **ret, Error **errp)
{
    ...
    if (visit_type_DumpQueryResult(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_DumpQueryResult(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_query_dump_guest_memory_capability(QDict *args, QObject **ret, Error **errp)
{
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_DumpGuestMemoryCapability(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_DumpGuestMemoryCapability(ov, "unused", &retval, NULL);
    visit_free(ov);
