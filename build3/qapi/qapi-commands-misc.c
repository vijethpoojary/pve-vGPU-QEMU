...
void qmp_marshal_query_name(QDict *args, QObject **ret, Error **errp)
{
    ...
    if (visit_type_NameInfo(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_NameInfo(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_query_iothreads(QDict *args, QObject **ret, Error **errp)
{
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_IOThreadInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_IOThreadInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_human_monitor_command(QDict *args, QObject **ret, Error **errp)
{
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_str(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_str(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_get_win32_socket(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    q_obj_get_win32_socket_arg arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_q_obj_get_win32_socket_arg_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_GET_WIN32_SOCKET)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_get_win32_socket(req_json->str);
    }

    qmp_get_win32_socket(arg.info, arg.fdname, &err);
    if (err) {
        trace_qmp_exit_get_win32_socket(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    trace_qmp_exit_get_win32_socket("{}", true);

out:
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_q_obj_get_win32_socket_arg_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
...
void qmp_marshal_add_fd(QDict *args, QObject **ret, Error **errp)
{
    ...
    retval = qmp_add_fd(arg.has_fdset_id, arg.fdset_id, arg.opaque, &err);
    if (err) {
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_AddfdInfo(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_AddfdInfo(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_query_fdsets(QDict *args, QObject **ret, Error **errp)
{
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_FdsetInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_FdsetInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_savevm_start(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    q_obj_savevm_start_arg arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_q_obj_savevm_start_arg_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_SAVEVM_START)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_savevm_start(req_json->str);
    }

    qmp_savevm_start(arg.statefile, &err);
    if (err) {
        trace_qmp_exit_savevm_start(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    trace_qmp_exit_savevm_start("{}", true);

out:
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_q_obj_savevm_start_arg_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}

void qmp_marshal_savevm_end(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_SAVEVM_END)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_savevm_end(req_json->str);
    }

    qmp_savevm_end(&err);
    if (err) {
        trace_qmp_exit_savevm_end(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    trace_qmp_exit_savevm_end("{}", true);
...
void qmp_marshal_query_command_line_options(QDict *args, QObject **ret, Error **errp)
{
    ...
    retval = qmp_query_command_line_options(arg.option, &err);
    if (err) {
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_CommandLineOptionInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_CommandLineOptionInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
