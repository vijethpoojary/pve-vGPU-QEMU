...
void qmp_marshal_query_version(QDict *args, QObject **ret, Error **errp)
{
    ...
    if (visit_type_VersionInfo(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_VersionInfo(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_query_commands(QDict *args, QObject **ret, Error **errp)
{
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_CommandInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_CommandInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
