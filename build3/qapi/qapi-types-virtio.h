...
typedef struct VirtIOGPUOutput VirtIOGPUOutput;

typedef struct IOThreadVirtQueueMappingList IOThreadVirtQueueMappingList;

typedef struct VirtIOGPUOutputList VirtIOGPUOutputList;
...
typedef enum VMAppleVirtioBlkVariant {
    VM_APPLE_VIRTIO_BLK_VARIANT_UNSPECIFIED,
    VM_APPLE_VIRTIO_BLK_VARIANT_ROOT,
    VM_APPLE_VIRTIO_BLK_VARIANT_AUX,
    VM_APPLE_VIRTIO_BLK_VARIANT__MAX,
} VMAppleVirtioBlkVariant;

#define VMAppleVirtioBlkVariant_str(val) \
    qapi_enum_lookup(&VMAppleVirtioBlkVariant_lookup, (val))

extern const QEnumLookup VMAppleVirtioBlkVariant_lookup;
...
struct VirtioDeviceFeatures {
    ...
    bool has_unknown_dev_features2;
    uint64_t unknown_dev_features2;
};
...
void qapi_free_VirtIOGPUOutput(VirtIOGPUOutput *obj);
G_DEFINE_AUTOPTR_CLEANUP_FUNC(VirtIOGPUOutput, qapi_free_VirtIOGPUOutput)
...
struct VirtIOGPUOutputList {
    VirtIOGPUOutputList *next;
    VirtIOGPUOutput *value;
};

void qapi_free_VirtIOGPUOutputList(VirtIOGPUOutputList *obj);
G_DEFINE_AUTOPTR_CLEANUP_FUNC(VirtIOGPUOutputList, qapi_free_VirtIOGPUOutputList)

struct DummyVirtioForceArrays {
    IOThreadVirtQueueMappingList *unused_iothread_vq_mapping;
    VirtIOGPUOutputList *unused_virtio_gpu_output;
};
