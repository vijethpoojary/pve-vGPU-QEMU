...
{
    if (trace_event_get_state(TRACE_QMP_EXIT_QUERY_ACPI_OSPM_STATUS)) {
        if (qemu_loglevel_mask(LOG_TRACE)) {
#line 4 "qapi/qapi-commands-acpi.trace-events"
            qemu_log("qmp_exit_query_acpi_ospm_status " "%s %d" "\n", result, succeeded);
#line 44 "trace/trace-qapi_commands_acpi_trace_events.h"
        }
    }
}
