...
void qmp_marshal_qom_list(QDict *args, QObject **ret, Error **errp)
{
    ...
    if (visit_type_ObjectPropertyInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_ObjectPropertyInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_qom_get(QDict *args, QObject **ret, Error **errp)
{
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_any(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_any(ov, "unused", &retval, NULL);
    visit_free(ov);
...
void qmp_marshal_qom_list_get(QDict *args, QObject **ret, Error **errp)
{
    Error *err = NULL;
    bool ok = false;
    Visitor *v;
    ObjectPropertiesValuesList *retval;
    Visitor *ov;
    q_obj_qom_list_get_arg arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_q_obj_qom_list_get_arg_members(v, &arg, errp)) {
        ok = visit_check_struct(v, errp);
    }
    visit_end_struct(v, NULL);
    if (!ok) {
    ...
    if (trace_event_get_state_backends(TRACE_QMP_ENTER_QOM_LIST_GET)) {
        g_autoptr(GString) req_json = qobject_to_json(QOBJECT(args));

        trace_qmp_enter_qom_list_get(req_json->str);
    }

    retval = qmp_qom_list_get(arg.paths, &err);
    if (err) {
        trace_qmp_exit_qom_list_get(error_get_pretty(err), false);
        error_propagate(errp, err);
        goto out;
    }

    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_ObjectPropertiesValuesList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_ObjectPropertiesValuesList(ov, "unused", &retval, NULL);
    visit_free(ov);

    if (trace_event_get_state_backends(TRACE_QMP_EXIT_QOM_LIST_GET)) {
        g_autoptr(GString) ret_json = qobject_to_json(*ret);

        trace_qmp_exit_qom_list_get(ret_json->str, true);
    }
...
out:
    ...
    v = qapi_dealloc_visitor_new();
    visit_start_struct(v, NULL, NULL, 0, NULL);
    visit_type_q_obj_qom_list_get_arg_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
...
void qmp_marshal_qom_list_types(QDict *args, QObject **ret, Error **errp)
{
    ...
    retval = qmp_qom_list_types(arg.implements, arg.has_abstract, arg.abstract, &err);
    if (err) {
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_ObjectTypeInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_ObjectTypeInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
