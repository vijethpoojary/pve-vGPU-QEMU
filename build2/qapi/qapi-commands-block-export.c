...
void qmp_marshal_nbd_server_start(QDict *args, QObject **ret, Error **errp)
{
    ...
    bool ok = false;
    Visitor *v;
    NbdServerOptionsLegacy arg = {0};

    v = qobject_input_visitor_new_qmp(QOBJECT(args));
    if (!visit_start_struct(v, NULL, NULL, 0, errp)) {
        goto out;
    }
    if (visit_type_NbdServerOptionsLegacy_members(v, &arg, errp)) {
    ...
    qmp_nbd_server_start(arg.has_handshake_max_seconds, arg.handshake_max_seconds, arg.tls_creds, arg.tls_authz, arg.has_max_connections, arg.max_connections, arg.addr, &err);
    if (err) {
...
out:
    ...
    visit_type_NbdServerOptionsLegacy_members(v, &arg, NULL);
    visit_end_struct(v, NULL);
    visit_free(v);
}
...
void qmp_marshal_query_block_exports(QDict *args, QObject **ret, Error **errp)
{
    ...
    if (visit_type_BlockExportInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_BlockExportInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
