...
bool visit_type_InetSocketAddress_members(Visitor *v, InetSocketAddress *obj, Error **errp)
{
    ...
    if (visit_optional(v, "keep-alive-count", &obj->has_keep_alive_count)) {
        if (!visit_type_uint32(v, "keep-alive-count", &obj->keep_alive_count, errp)) {
            return false;
        }
    }
#endif /* defined(HAVE_TCP_KEEPCNT) */
#if defined(HAVE_TCP_KEEPIDLE)
    if (visit_optional(v, "keep-alive-idle", &obj->has_keep_alive_idle)) {
        if (!visit_type_uint32(v, "keep-alive-idle", &obj->keep_alive_idle, errp)) {
            return false;
        }
    }
#endif /* defined(HAVE_TCP_KEEPIDLE) */
#if defined(HAVE_TCP_KEEPINTVL)
    if (visit_optional(v, "keep-alive-interval", &obj->has_keep_alive_interval)) {
        if (!visit_type_uint32(v, "keep-alive-interval", &obj->keep_alive_interval, errp)) {
            return false;
        }
    }
